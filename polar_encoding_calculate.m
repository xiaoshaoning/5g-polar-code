...
row_weights_valid = row_weights(q_tilt_list);
min_row_weights = min(row_weights_valid);

if n_wm_pc == 1
    for index = q_info_size-n_pc:-1:1
        if row_weights(q_tilt_list(index)) == min_row_weights
            q_pc_list = [q_info_list(1:(n_pc-1)), q_tilt_list(index)];
        end
    end
else % n_wm_pc == 0
